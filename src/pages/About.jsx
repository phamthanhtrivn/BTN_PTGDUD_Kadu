  ...
  return (
    <>
      <div className="flex flex-col md:flex-row gap-4 px-4">
        ...
        <div className="sidebar hidden md:block md:w-[30%] flex flex-col gap-4">
          <h1 className="my-5 font-bold text-xl">Bài viết mới nhất</h1>
          {articles.map((article, index) => (
            <SidebarArticleItem key={index} article={article} onClick={() => hanleClick(article.id)} />
          ))}
        </div>
      </div>

      <h1 className="my-5 font-bold text-2xl">Bài viết liên quan</h1>
      <div className="xem-them grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-4 gap-3">
        {articles.slice(0, 4).map((article, index) => (
          <ArticleCard key={index} article={article} onClick={() => hanleClick(article.id)} />
        ))}
      </div>
    </>
  )
