...
function Cart() {
  const { products, currency, cartItems, setCartItems, navigate, formatMoney, delivery_fee, handleUpdateQuantityCart, handleDeleteCartItem } =
  ...
  return (
    <div className="">
      <div className="flex flex-col sm:flex-row sm:justify-between gap-5">
        ...
        {cartData.length > 0 ? (<button onClick={() => handleDeleteAll()} className="w-[150px] px-4.5 py-2 hover:bg-red-600 hover:text-white rounded border border-red-600 bg-white text-red-600 transition-all duration-300">Xóa tất cả</button>) : null}
      </div>
        ...
        {cartData.length > 0 ? (
          <>
            {cartData.map((item, index) => (
              ...
              >
                <div onClick={() => navigate(`/product/${item.id}`)} className="flex items-start gap-6 cursor-pointer">
                ...
                <input
                  ...
                  onChange={(e) => handleUpdateQuantityCart(item.id, Number(e.target.value))}
                />
                <p>
                  {formatMoney(item.price * item.quantity)} {currency}{" "}
                </p>
                <i className="fa-solid fa-trash-can cursor-pointer text-red-600" onClick={() => handleDelete(item.id)}></i>
              </div>
            ))}
          </>
        ) : (
        ...
        <div className="flex justify-end my-10">
          <div className="w-full sm:w-[450px]">
            ...
            <div className="w-full text-end">
              <button onClick={() => navigate('/place-order')} className="font-medium bg-[#005E4F] text-white text-sm my-8 px-8 py-3 cursor-pointer border border-[#005E4F] hover:text-[#005E4F] hover:bg-white transition-all duration-300 rounded">
                THANH TOÁN
              </button>
            </div>
          </div>
